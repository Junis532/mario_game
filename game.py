...
paused = False
alpha = 0
red_screen = pygame.Surface((800, 600))
red_screen.fill((255, 0, 0))  # 빨간색으로 채우기
...
class Player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.sit_animation_speed = 0.1

    def read_keyword_from_file(self, filename):
        try:
    ...
    def player_input(self):
        ...
        space_keyword = self.read_keyword_from_file("space.txt")
        shift_keyword = self.read_keyword_from_file("shift.txt")
        ...
        elif space_keyword == "space" and self.rect.bottom >= 300:
            self.gravity = -16.25
            self.jump_sound.play()
        elif shift_keyword == "shift":
            self.sitting = True
            self.rect = self.player_sit_1.get_rect(midbottom=self.rect.midbottom)
        else:
            self.sitting = False
...
class Heart(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        heart_image = pygame.image.load('graphics/heart/heart.png').convert_alpha()
...
def collision_sprite():   #충돌판정함수
    global paused
    global lifepoint
    global alpha
    global obstacle_speed
    
    if not paused:  # 일시 중지 중이 아닌 경우에만 실행
...
smaller_font = pygame.font.Font('font/Pixeltype.ttf', 40)
game_active = False
...
bestscore_sound = pygame.mixer.Sound('audio/bestscore.mp3')
...
heart_timer = pygame.USEREVENT + 3  # 새로운 타이머 사용
pygame.time.set_timer(coin_timer, 3000)  # 3초로 설정
pygame.time.set_timer(heart_timer, 15000)  # 15초로 설정
...
game_message = test_font.render('Press SHIFT to start', False, (240, 230, 140))
...
while True:
        ...
        if game_active:
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_RSHIFT:
                if endcount == 1:
                    ...
                    coin_group.empty()
                    heart_group.empty()
                    obstacle_speed = 3  # obstacle_speed를 2.5로 초기화
    ...
    if game_active:
        ...
        red_screen.set_alpha(alpha)  # 반투명 설정
        ...
        current_time = int(pygame.time.get_ticks() / 1000) - start_time
        total_score = score + current_time

        score_text = test_font.render(f'Score: {total_score}', False, (64, 64, 64))
        score_text_rect = score_text.get_rect(center=(400, 40))
        screen.blit(score_text, score_text_rect)

        life_text = test_font.render(f'Life: {lifepoint}', False, (64, 64, 64))
        life_text_rect = life_text.get_rect(center=(80, 40))
        ...
        for heart in heart_group:
            heart.rect.x -= int(obstacle_speed)
        ...
        heart_collisions = pygame.sprite.spritecollide(player.sprite, heart_group, True)
        if heart_collisions:
            if lifepoint < 3:
                lifepoint += 1
                heart_sound.play()
        ...
        instagram_text = smaller_font.render(f'Instagram: @wal._.foo', False, (255, 255, 255))
        instagram_rect = instagram_text.get_rect(bottomright=(800, 400))
        screen.blit(instagram_text, instagram_rect)
